...
typedef enum ks_err {
  ...
  KS_ERR_ASM_MACRO_LEVELS_EXCEED, // macros cannot be nested more than 20 levels
                                  // deep
...
// In that case, ks_asm() would eventually return with error
...
/*
 ...
 @encoding: array of bytes containing encoding of input assembly string.
           NOTE: *encoding will be allocated by this function, and should be
 freed with ks_free() function.
...
*/
KEYSTONE_EXPORT
int ks_asm(ks_engine *ks, const char *string, uint64_t address,
           unsigned char **encoding, size_t *encoding_size, size_t *stat_count);
