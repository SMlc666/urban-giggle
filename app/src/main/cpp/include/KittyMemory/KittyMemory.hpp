...
#define KT_PAGE_LEN2(x, len)                                                   \
  (KT_PAGE_END2(x, len) - KT_PAGE_START(x) + KT_PAGE_SIZE)
...
#define KITTY_LOGD(fmt, ...)                                                   \
  ((void)__android_log_print(ANDROID_LOG_DEBUG, KITTY_LOG_TAG, fmt,            \
                             ##__VA_ARGS__))
#else
#define KITTY_LOGD(fmt, ...)                                                   \
  do {                                                                         \
...
#define KITTY_LOGI(fmt, ...)                                                   \
  ((void)__android_log_print(ANDROID_LOG_INFO, KITTY_LOG_TAG, fmt,             \
                             ##__VA_ARGS__))
#define KITTY_LOGE(fmt, ...)                                                   \
  ((void)__android_log_print(ANDROID_LOG_ERROR, KITTY_LOG_TAG, fmt,            \
                             ##__VA_ARGS__))
...
#ifdef kITTYMEMORY_DEBUG
#define KITTY_LOGD(fmt, ...)                                                   \
  os_log(OS_LOG_DEFAULT, "D " KITTY_LOG_TAG ": " fmt, ##__VA_ARGS__)
#else
#define KITTY_LOGD(fmt, ...)                                                   \
  do {                                                                         \
  } while (0)
...
#define KITTY_LOGI(fmt, ...)                                                   \
  os_log(OS_LOG_DEFAULT, "I " KITTY_LOG_TAG ": " fmt, ##__VA_ARGS__)
#define KITTY_LOGE(fmt, ...)                                                   \
  os_log_error(OS_LOG_DEFAULT, "E " KITTY_LOG_TAG ": " fmt, ##__VA_ARGS__)
...
public:
  ...
  size_t length;
  int protection;
  bool readable, writeable, executable, is_private, is_shared, is_ro, is_rw,
      is_rx;
  ...
  ProcMap()
      : startAddress(0), endAddress(0), length(0), protection(0),
        readable(false), writeable(false), executable(false), is_private(false),
        is_shared(false), is_ro(false), is_rw(false), is_rx(false), offset(0),
        inode(0) {}
  ...
  inline bool isValidELF() const {
    return isValid() && length > 4 && readable &&
           memcmp((const void *)startAddress, "\177ELF", 4) == 0;
  }
  inline bool contains(uintptr_t address) const {
    return address >= startAddress && address < endAddress;
  }
  inline std::string toString() {
    return KittyUtils::String::Fmt(
        "%llx-%llx %c%c%c%c %llx %s %lu %s", startAddress, endAddress,
 ...
 */
std::vector<ProcMap> getMapsEqual(const std::vector<ProcMap> &maps,
                                  const std::string &name);

/*
 * Gets info of all maps which pathname contains @name in current process
 */
std::vector<ProcMap> getMapsContain(const std::vector<ProcMap> &maps,
                                    const std::string &name);

/*
 * Gets info of all maps which pathname ends with @name in current process
 */
std::vector<ProcMap> getMapsEndWith(const std::vector<ProcMap> &maps,
                                    const std::string &name);
 ...
 * Gets info of all maps which pathname equals @name in current process
 */
inline std::vector<ProcMap> getMapsEqual(const std::string &name) {
  return getMapsEqual(getAllMaps(), name);
}

/*
 * Gets info of all maps which pathname contains @name in current process
 */
inline std::vector<ProcMap> getMapsContain(const std::string &name) {
  return getMapsContain(getAllMaps(), name);
}

/*
 * Gets info of all maps which pathname ends with @name in current process
 */
inline std::vector<ProcMap> getMapsEndWith(const std::string &name) {
  return getMapsEndWith(getAllMaps(), name);
}

/*
 * Gets map info of an address in self process
 */
inline ProcMap getAddressMap(const void *address) {
...
public:
  ...
  inline seg_data_t getSection(const char *seg_name,
                               const char *sect_name) const {
    seg_data_t data{};
    if (!header || !seg_name || !sect_name)
      return data;
    data.start =
        uintptr_t(getsectiondata(header, seg_name, sect_name, &data.size));
    data.end = data.start + data.size;
    return data;
  }
};
...
kern_return_t getPageInfo(vm_address_t region,
                          vm_region_submap_short_info_64 *info_out);
...
/*
 * find in memory file info by checking if target loaded object file ends with
 * @fileName
 */
MemoryFileInfo getMemoryFileInfo(const std::string &fileName);

/*
 * returns the absolue address of a relative offset of a file in memory or NULL
 * as fileName for base executable
 */
...
uintptr_t findSymbol(const KittyMemory::MemoryFileInfo &info,
