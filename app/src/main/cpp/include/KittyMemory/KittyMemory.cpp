...
bool memRead(const void *address, void *buffer, size_t len) {
  ...
  if (setAddressProtection(address, len, addressMap.protection | PROT_READ) !=
      0) {
    KITTY_LOGE("memRead err couldn't add write perm to address (%p, len: %zu, "
               "prot: %d)",
  ...
  if (setAddressProtection(address, len, addressMap.protection) != 0) {
    KITTY_LOGE("memRead err couldn't revert protection of address (%p, len: "
               "%zu, prot: %d)",
               address, len, addressMap.protection);
    return false;
  }
...
bool memWrite(void *address, const void *buffer, size_t len) {
  ...
  if (setAddressProtection(address, len, addressMap.protection | PROT_WRITE) !=
      0) {
    KITTY_LOGE("memWrite err couldn't add write perm to address (%p, len: %zu, "
               "prot: %d)",
               address, len, addressMap.protection);
    return false;
  }
  ...
  if (setAddressProtection(address, len, addressMap.protection) != 0) {
    KITTY_LOGE("memWrite err couldn't revert protection of address (%p, len: "
               "%zu, prot: %d)",
               address, len, addressMap.protection);
    return false;
  }
...
std::vector<ProcMap> getAllMaps() {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    sscanf(line, "%llx-%llx %s %llx %s %lu %s", &map.startAddress,
           &map.endAddress, perms, &map.offset, dev, &map.inode, pathname);
...
std::vector<ProcMap> getMapsContain(const std::vector<ProcMap> &maps,
  ...
  for (auto &it : maps) {
    if (it.isValid() && !it.isUnknown() &&
        strstr(it.pathname.c_str(), name.c_str())) {
      retMaps.push_back(it);
    }
  }
...
kern_return_t getPageInfo(vm_address_t region,
  ...
  return vm_region_recurse_64(mach_task_self(), &region, &region_len, &depth,
                              (vm_region_recurse_info_t)info_out, &info_count);
}
...
bool memRead(const void *address, void *buffer, size_t len) {
  ...
  kern_return_t kret = mach_vm_read_overwrite(
      mach_task_self(), mach_vm_address_t(address), mach_vm_size_t(len),
...
Memory_Status memWrite(void *address, const void *buffer, size_t len) {
  ...
  if (kret != KERN_SUCCESS) {
    KITTY_LOGE(
        "memWrite err failed to get page info of address (%p) - kerror(%d).",
        address, kret);
  ...
  if (page_info.protection & VM_PROT_WRITE) {
    kret = mach_vm_write(self_task, mach_vm_address_t(address),
  ...
  if (kret != KERN_SUCCESS) {
    KITTY_LOGE("memWrite err vm_protect(page: %p, len: %zu, prot: %d) COW "
               "failed - kerror(%d).",
  ...
  if (kret != KERN_SUCCESS) {
    KITTY_LOGE("memWrite err vm_protect(page: %p, len: %zu, prot: %d) restore "
               "failed - kerror(%d).",
               (void *)page_start, page_len, page_info.protection, kret);
    return KMS_ERR_PROT;
  }
...
MemoryFileInfo getBaseInfo() {
  ...
  for (uint32_t i = 0; i < imageCount; i++) {
    ...
    if (!name || strlen(name) != strlen(exeBuf.data()) ||
        strcmp(name, exeBuf.data()) != 0)
      continue;
...
uintptr_t findSymbol(const KittyMemory::MemoryFileInfo &info,
  ...
  for (uint32_t i = 0; i < header->ncmds; i++, curr += curr_seg_cmd->cmdsize) {
    *(uintptr_t *)&curr_seg_cmd = curr;
    ...
    if (curr_seg_cmd->cmd == lc_seg &&
        (strcmp(curr_seg_cmd->segname, SEG_LINKEDIT) == 0))
  ...
  uintptr_t linkedit_base =
      (slide + linkedit_segment_cmd->vmaddr) - linkedit_segment_cmd->fileoff;
