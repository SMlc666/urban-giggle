...
uintptr_t findInRange(const uintptr_t start, const uintptr_t end,
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (!compare(reinterpret_cast<const char *>(start + i), pattern,
                 mask.c_str()))
      continue;
...
uintptr_t findBytesFirst(const uintptr_t start, const uintptr_t end,
...
std::vector<uintptr_t> findHexAll(const uintptr_t start, const uintptr_t end,
                                  std::string hex, const std::string &mask) {
  std::vector<uintptr_t> list;
...
uintptr_t findHexFirst(const uintptr_t start, const uintptr_t end,
                       std::string hex, const std::string &mask) {
  if (start >= end || mask.empty() || !KittyUtils::String::ValidateHex(hex))
    return 0;
...
std::vector<uintptr_t> findIdaPatternAll(const uintptr_t start,
                                         const uintptr_t end,
  ...
  for (std::size_t i = 0; i < pattren_len; i++) {
    ...
    if (pattern[i] == '?') {
      bytes.push_back(0);
      mask += '?';
    } else if (pattren_len > i + 1 && std::isxdigit(pattern[i]) &&
...
uintptr_t findIdaPatternFirst(const uintptr_t start, const uintptr_t end,
                              const std::string &pattern) {
  if (start >= end)
    return 0;
...
std::vector<uintptr_t> findDataAll(const uintptr_t start, const uintptr_t end,
...
uintptr_t findDataFirst(const uintptr_t start, const uintptr_t end,
                        const void *data, size_t size) {
  if (start >= end || !data || size < 1)
    return 0;
...
RegisterNativeFn findRegisterNativeFn(const class ElfScanner &elf,
  ...
  string_loc = KittyScanner::findDataFirst(elf.baseSegment().startAddress,
                                           elf.baseSegment().endAddress,
                                           name.data(), name.length());
  if (!string_loc) {
    KITTY_LOGE(
        "findRegisterNativeFn: Couldn't find string (%s) in selected maps",
        name.c_str());
    return fn;
  }

  KITTY_LOGD("findRegisterNativeFn: String (%s) at %p", name.c_str(),
             (void *)string_loc);

  for (auto &it : elf.segments()) {
    if (it.is_rw) {
      string_xref = KittyScanner::findDataFirst(it.startAddress, it.endAddress,
                                                &string_loc, sizeof(uintptr_t));
      if (!string_xref)
        continue;

      KITTY_LOGD("findRegisterNativeFn: String at (%p) referenced at %p",
...
ElfScanner::ElfScanner(uintptr_t elfBase,
                       const std::vector<KittyMemory::ProcMap> &maps) {
  _elfBase = 0;
  _ehdr = {};
  _phdr = 0;
  _loads = 0;
  ...
  if (!elfBaseMap.isValid() || !elfBaseMap.readable ||
      elfBase != elfBaseMap.startAddress) {
    KITTY_LOGD("ElfScanner: (%p) is not a valid ELF base address.",
               (void *)elfBase);
    return;
  }
  ...
  if (!_ehdr.e_phnum || !_ehdr.e_phentsize || !_ehdr.e_shnum ||
      !_ehdr.e_shentsize) {
  ...
  if (!memcpy(&phdrs_buf[0], (const void *)_phdr, phdrs_buf.size())) {
    KITTY_LOGD("ElfScanner: Failed to read ELF (%p) program headers.",
               (void *)elfBase);
    return;
  }
  ...
  for (ElfW_(Half) i = 0; i < _ehdr.e_phnum; i++) {
    ...
    memcpy(&phdr_entry, phdrs_buf.data() + (i * _ehdr.e_phentsize),
           _ehdr.e_phentsize);
  ...
  if (!max_vaddr) {
    KITTY_LOGD("ElfScanner: Failed to find load size for ELF (%p).",
               (void *)elfBase);
    return;
  }
  ...
  for (auto &phdr : _phdrs) {
    if (phdr.p_type == PT_DYNAMIC) {
      ...
      if (!memcpy(&dyn_buff[0], (const void *)_dynamic, phdr.p_memsz)) {
        KITTY_LOGD("ElfScanner: Failed to read dynamic for ELF (%p).",
                   (void *)elfBase);
        break;
      }
  ...
  if (!_stringTable || !_symbolTable || !_strsz || !_syment) {
    ...
    KITTY_LOGD("ElfScanner: elfBase: %p | strtab=%p | symtab=%p | strsz=%p | "
               "syment=%p",
               (void *)elfBase, (void *)_stringTable, (void *)_symbolTable,
               (void *)_strsz, (void *)_syment);
    return;
  }
  ...
  for (auto &it : maps) {
    if (it.startAddress >= _elfBase &&
        it.endAddress <= (_elfBase + _loadSize)) {
...
uintptr_t ElfScanner::findSymbol(const std::string &symbolName) const {
  ...
  auto get_sym_address = [&](const ElfW_(Sym) * sym_ent) -> uintptr_t {
    return sym_ent->st_value < _loadBias ? _loadBias + sym_ent->st_value
                                         : sym_ent->st_value;
  };

  // try gnu hash first
  if (_gnuHashTable) {
    const auto *sym = KittyUtils::Elf::GnuHash::LookupByName(
        _gnuHashTable, _symbolTable, _stringTable, _syment, _strsz,
        symbolName.c_str());
    if (sym && sym->st_value) {
  ...
  if (_elfHashTable) {
    const auto *sym = KittyUtils::Elf::ElfHash::LookupByName(
        _elfHashTable, _symbolTable, _stringTable, _syment, _strsz,
        symbolName.c_str());
    if (sym && sym->st_value) {
      return get_sym_address(sym);
    }
...
std::vector<ElfScanner> ElfScanner::getAllELFs() {
  ...
  for (auto &it : maps) {
    ...
    if (!strstr(it.pathname.c_str(), "/bin/linker")) {
      ...
      // re-assigning the pathname in case when library is zipped inside
      // base.apk dli_fname returns basename sometimes, so check basename before
      // re-assigning the full pathname
...
std::vector<std::pair<uintptr_t, ElfScanner>>
ElfScanner::findSymbolAll(const std::string &symbolName) {
