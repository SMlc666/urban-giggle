...
const ElfW_(Sym) * LookupByName(uintptr_t elfhash, uintptr_t symtab,
                                uintptr_t strtab, size_t syment, size_t strsz,
  ...
  for (uint32_t i = bucket[name_hash % num_bucket]; i != 0 && i < num_chain;
       i = chain[i]) {
    const auto *symbol =
        reinterpret_cast<const ElfW_(Sym) *>(symbol_table + (syment * i));
...
const ElfW_(Sym) * LookupByName(uintptr_t gnuhash, uintptr_t symtab,
                                uintptr_t strtab, size_t syment, size_t strsz,
                                const char *symbol_name) {
  ...
  uintptr_t mask = 0 | (uintptr_t)1 << (name_hash % ELFCLASS_BITS_) |
                   (uintptr_t)1
                       << ((name_hash >> bloom_shift) % ELFCLASS_BITS_);
  ...
  while (true) {
    const auto *symbol =
        reinterpret_cast<const ElfW_(Sym) *>(symbol_table + (syment * sym_idx));
    if (!symbol || symbol->st_name >= strsz)
      break;
